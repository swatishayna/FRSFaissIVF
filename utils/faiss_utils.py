...
def load_faiss_index(index_path, dimension=512, nlist=1000):
    """
    Load an existing FAISS IVF index or create a new one if not found.
    """
    try:
        if os.path.exists(index_path):
            index = faiss.read_index(index_path)
            print(f"✅ Loaded existing FAISS IVF index from: {index_path}")
        else:
            print("🆕 Creating new FAISS IVF index...")
            quantizer = faiss.IndexFlatL2(dimension)
            index = faiss.IndexIVFFlat(quantizer, dimension, nlist, faiss.METRIC_L2)
            print(f"✅ Created new IVF index (dim={dimension}, nlist={nlist})")
        return index
...
def add_to_faiss_index(index, embeddings):
    """
    Add embeddings to a FAISS IVF index (auto-trains if needed).
    """
    try:
        ...
        # If IVF index is untrained, train before adding
        ...
        print(f"✅ Added {embeddings.shape[0]} embeddings to FAISS IVF index.")
        return index
    except Exception as e:
        print(f"❌ Error adding embeddings to FAISS IVF index: {e}")
        raise e
...
def save_faiss_index(index, index_path):
    ...
    try:
        ...
        print(f"💾 FAISS IVF index saved at: {index_path}")
    except Exception as e:
...
def search_faiss_index(index, query_embedding, top_k=5):
    """
    Search for similar embeddings in the FAISS IVF index.
    """
    try:
...
def force_rebuild_faiss_index(
    ...
    faiss_index_path="data/faiss/faiss_ivf.index",
